...
import type { Industry, UserType, ProjectFormData, UserProfile, IntakeDetails, DeliverableType } from "@/types";
...
// ============================
// مولّد عام: أي مستند نصّي → مخطّط موحّد { title, subtitle, sections[] }
// يضمن محتوى مخصّصاً ومميّزاً لكل نوع مخرج بدل تكرار قالب واحد.
// ============================
const DOC_GUIDANCE: Partial<Record<DeliverableType, { title: string; sections: string }>> = {
  scope_statement: {
    title: "بيان نطاق المشروع",
    sections: "وصف نطاق المنتج، المخرجات الرئيسية (قائمة)، ما هو داخل النطاق (قائمة)، ما هو خارج النطاق صراحةً (قائمة)، معايير القبول (قائمة)، القيود (قائمة)، الافتراضات (قائمة).",
  },
  stakeholder_register: {
    title: "سجل أصحاب المصلحة",
    sections: "القسم الأساسي جدول سجل أصحاب المصلحة (جدول: الاسم/الجهة | الدور | مستوى التأثير | مستوى الاهتمام | استراتيجية الإشراك والتواصل) — أدرج كل صاحب مصلحة من بيانات المشروع في صف مستقل. ثم قسم تصنيف أصحاب المصلحة حسب مصفوفة التأثير/الاهتمام (نص موجز)، ثم ملخص خطة التواصل لكل فئة (قائمة). ركّز على أصحاب المصلحة أنفسهم — لا تكتب نظرة عامة عن المشروع.",
  },
  resource_plan: {
    title: "خطة إدارة الموارد",
    sections: "نظرة عامة على احتياج الموارد (نص — يجب أن يتّسق مع فترات الأدوار في الجدول، لا تقل إن الجميع 100% طوال المدة إن كانت الفترات مختلفة)، الهيكل التنظيمي وخطوط رفع التقارير (نص موجز يصف من يرفع لمن)، الأدوار والمسؤوليات والتخصيص (جدول: الدور | العدد | المسؤوليات | نسبة التخصيص | الفترة)، الموارد المادية والتقنية (قائمة)، خطة اكتساب الفريق (نص)، مصفوفة المسؤوليات RACI (جدول) — استخدم فقط الرموز R و A و C و I في خلايا المصفوفة، خطة إطلاق/تسريح الموارد بعد كل مرحلة (قائمة)، ميثاق الفريق وقواعد العمل (قائمة)، التقدير والمكافآت (نص موجز).",
  },
  risk_response: {
    title: "خطة الاستجابة للمخاطر",
    sections: "نظرة عامة على استراتيجية الاستجابة (نص)، استراتيجيات مخاطر التهديد (جدول: الخطر | الاستراتيجية | الإجراءات | المسؤول)، استراتيجيات الفرص (جدول)، خطط الطوارئ والاحتياطيات (قائمة)، آلية المتابعة والمراجعة (نص).",
  },
  quality_plan: {
    title: "خطة إدارة الجودة",
    sections: "سياسة الجودة (نص)، معايير ومقاييس الجودة (جدول: المقياس | الهدف | طريقة القياس)، أنشطة ضمان الجودة (قائمة)، أنشطة ضبط الجودة (قائمة)، الأدوار والمسؤوليات في الجودة (جدول)، خطة التحسين المستمر (نص).",
  },
  communication_plan: {
    title: "خطة التواصل",
    sections: "أهداف التواصل (نص)، مصفوفة التواصل (جدول: صاحب المصلحة | المعلومة | التكرار | القناة | المسؤول)، آلية التصعيد (نص)، اجتماعات المشروع الدورية (جدول)، إدارة التقارير (قائمة).",
  },
  status_report: {
    title: "تقرير حالة المشروع",
    sections: "الملخص التنفيذي (نص)، مؤشرات الحالة العامة: الجدول/الميزانية/النطاق/الجودة (جدول)، إنجازات هذه الفترة (قائمة)، الأنشطة القادمة (قائمة)، المخاطر والمشكلات الحالية (جدول)، طلبات القرار (قائمة).",
  },
  meeting_minutes: {
    title: "محضر اجتماع",
    sections: "معلومات الاجتماع (زوج مفتاح-قيمة: التاريخ، المكان، مدير الاجتماع)، الحضور (قائمة)، جدول الأعمال (قائمة)، النقاشات والقرارات (قائمة)، بنود العمل (جدول: البند | المسؤول | الموعد النهائي)، الاجتماع القادم (نص).",
  },
  closure_report: {
    title: "تقرير إغلاق المشروع",
    sections: "الملخص التنفيذي للإغلاق (نص)، تحقيق الأهداف والنطاق (جدول: الهدف | الحالة)، الأداء النهائي للجدول والميزانية (جدول)، المخرجات المسلّمة والمقبولة (قائمة)، الدروس المستفادة الرئيسية (قائمة)، البنود المتبقية والتوصيات (قائمة)، إقرار الإغلاق (نص).",
  },
  lessons_learned: {
    title: "الدروس المستفادة",
    sections: "نظرة عامة (نص)، ما سار بشكل جيد (قائمة)، ما يمكن تحسينه (قائمة)، الدروس حسب المجال (جدول: المجال | الدرس | التوصية)، توصيات للمشاريع المستقبلية (قائمة).",
  },
  milestone_chart: {
    title: "مخطط المعالم",
    sections: "نظرة عامة على المعالم (نص)، جدول المعالم (جدول: المعلم | الأسبوع/التاريخ | الوصف | معيار النجاح)، المعالم الحرجة (قائمة).",
  },
  quality_checklist: {
    title: "قائمة فحص الجودة",
    sections: "الغرض (نص)، بنود الفحص حسب المرحلة (جدول: البند | المعيار | الحالة | ملاحظات)، معايير القبول النهائية (قائمة).",
  },
  closure_checklist: {
    title: "قائمة إغلاق المشروع",
    sections: "الغرض (نص)، بنود الإغلاق الإداري (جدول: البند | الحالة | المسؤول)، بنود الإغلاق التعاقدي (جدول)، بنود الأرشفة والتسليم (قائمة).",
  },
  earned_value: {
    title: "تحليل القيمة المكتسبة",
    sections: "نظرة عامة على منهجية EVM (نص)، المؤشرات الرئيسية (جدول: المؤشر | القيمة | التفسير) شاملة PV/EV/AC/CPI/SPI، تحليل الانحرافات (نص)، التنبؤ بالإكمال EAC/ETC (جدول)، التوصيات (قائمة).",
  },
};
...
export function buildDocumentPrompt(
  ...
  const industry = (project as { industry?: Industry }).industry ?? "other";
  const industryCtx = INDUSTRY_CONTEXT[industry] ?? INDUSTRY_CONTEXT.other;
  const g = DOC_GUIDANCE[type] ?? { title: "مستند المشروع", sections: "أقسام مناسبة للمستند بناءً على بيانات المشروع." };
  return {
    system: PMBOK_SYSTEM_CONTEXT,
    user: `
أنشئ مستند «${g.title}» احترافياً وفق معايير PMI/PMBOK للمشروع: ${project.name}
...
يجب أن يحتوي المستند على الأقسام التالية:
${g.sections}

أخرج JSON بالمخطّط الموحّد التالي حصراً (كل المحتوى بالعربية الفصحى):
{
  "title": "${g.title}",
  "subtitle": "سطر وصفي موجز",
  "sections": [
    { "heading": "عنوان القسم", "kind": "text", "text": "فقرة نصية" },
    { "heading": "عنوان القسم", "kind": "list", "items": ["عنصر 1", "عنصر 2"] },
    { "heading": "عنوان القسم", "kind": "table", "headers": ["عمود 1", "عمود 2"], "rows": [["قيمة", "قيمة"]] },
    { "heading": "عنوان القسم", "kind": "keyvalue", "pairs": [["المفتاح", "القيمة"]] }
  ]
}

قواعد صارمة (الاختصار مهم جداً لتفادي بتر الاستجابة):
- استخدم بيانات المشروع الحقيقية (أصحاب المصلحة، المخاطر، المعالم، الميزانية) ولا تخترع.
- عدد الأقسام: من 4 إلى 6 أقسام كحد أقصى — لا أكثر.
- كل قائمة (list): من 3 إلى 6 عناصر كحد أقصى.
- كل جدول (table): من 3 إلى 8 صفوف كحد أقصى، وعدد الأعمدة 5 كحد أقصى.
- كل خلية أو عنصر قائمة: 12 كلمة كحد أقصى.
- كل فقرة نصية (text): جملتان إلى ثلاث جُمل كحد أقصى.
- محدّد وموجز — لا حشو ولا تكرار بين الأقسام.
- استخدم "kind" المناسب لكل قسم (جدول للبيانات المهيكلة، قائمة للنقاط، نص للفقرات).
- أخرج JSON صالحاً ومكتملاً فقط بدون أي نص خارجه.
`.trim(),
  };
}
